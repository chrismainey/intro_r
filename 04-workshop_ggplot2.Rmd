...
Plots are composed of layers, and you can add them together (usinf `+`) to build plots

.pull-left[

+ A `ggplot` has:

  + a default dataset and sets of mappings from variables to aesthetics `aes`
  + one or more layers the draw Geometric objects `geom_`

    
+ They may also use:

  + A separate dataset and set of aesthetic mappings:
  + A scale for each aesthetic mapping, controlling axes, colours, linetypes etc.
  + A facet (panel) specification
  + A theme - styling elements
]

.pull-right[
<br>
```{r ggplotshell, eval = FALSE, include=TRUE}
ggplot(my_data_frame,
       aes(y=column_name1, x=column_name2))+
  geom_point(aes(fill=column_name3)
             , colour = "black") +
  scale_y_continuous(limits = c(0, 150))+
  facet_wrap(~column_name3)+
