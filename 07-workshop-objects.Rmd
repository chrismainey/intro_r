...
<br>
These data types can be held in several structures.  Fundamentally, you need to know:

+ `vector`:  An element of one data type. Created with the concatenate (`c()`) function:
```{r vector, echo=TRUE, eval=TRUE}
my_vector <- c(1,2,6,32,5,6)
```

<br>
+ `data.frame`: a rectangular table structure, similar to a database table.
```{r dataframe, echo=TRUE, eval=TRUE}
my_data_frame <- 
  data.frame(number_col = c(1,2,6,32,5,6)
             , logical_col = c(TRUE, FALSE, FALSE,TRUE,FALSE,TRUE)
